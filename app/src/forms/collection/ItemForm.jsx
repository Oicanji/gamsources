...
import { attrNames, extraNames, subType, typesFiles } from "../../types/items";
import { useParams } from "react-router";

export function ItemForm({ item, key, credits, rewriteItem }) {
  const { thisTheme } = useContext(ThemeContext);
  const [itemForm] = Form.useForm();
  const { id } = useParams();

  const [type, _] = useState(subType[item.type]);
  const [subtype, setSubtype] = useState(item.type);
    ...
    >
      <Row>
        <Col span={8}>
          ...
          >
            <FontAwesomeIcon icon={faFile} /> Item Preview
          </Divider>
          {type == "image" ? (
            !item.noOpen ? (
              <img
                height="200px"
                alt="preview"
                src={item.ref.replace("api", import.meta.env.VITE_API)}
        ...
        >
          ...
          <Form
            ...
            initialValues={{
              ...
              sensitive: item.sensitive_content
                ? item.sensitive_content
                : false,
              type: item.type ? item.type : subtype,
              ia: item.is_ia ? item.is_ia : false,

              extra: item.extra && attrNames[subtype] ? item.extra : null,
              attr: item.attr && attrNames[subtype] ? item.attr : null,
            }}
          >
            <Row>
              <Col span={12}>
